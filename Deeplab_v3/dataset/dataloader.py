...
class Image_loader(Dataset):
    ...
    def __init__(self, num_classes=3,mode='train'):
        self.num_classes = num_classes
        self.mode=mode
       
        #Normalization
        self.normalize = transforms.Compose([
            transforms.Resize((img_hight,img_width)),
        ...
        self.DATA_PATH =  'D:/Documents/Researches/Self_Driving_Car/datasetRoad'
        
        self.train_path = self.DATA_PATH + '/Train/images/'
        self.label_path = self.DATA_PATH + '/Train/labels/'
        #print(self.train_path)

        if self.mode == 'train':
            self.data_files = os.listdir(self.train_path)
            self.data_files.sort(key= lambda i: int(i.lstrip('data').rstrip('.png')))
    ...
    def image_loader(self, data_path, label_path):
        ...
        print(label.shape)
        return data, label.resize((img_width,img_hight))
